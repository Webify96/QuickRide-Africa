...
function cap(str) {
  return str
    ? str.replace(/-/g, " ").replace(/\b\w/g, function (c) {
...
function row(label, value) {
  ...
  return (
    "<tr>" +
    '<td style="padding:9px 14px;font-size:13px;color:#6b7280;font-family:Arial,sans-serif;width:170px;vertical-align:top;border-bottom:1px solid #f3f4f6;">' +
    label +
    "</td>" +
    '<td style="padding:9px 14px;font-size:14px;color:#111827;font-family:Arial,sans-serif;vertical-align:top;border-bottom:1px solid #f3f4f6;">' +
    value +
    "</td>" +
    "</tr>"
  );
...
function section(title, rows) {
  ...
  return (
    ...
    '<span style="font-size:11px;font-weight:700;color:#fff;letter-spacing:0.1em;text-transform:uppercase;font-family:Arial,sans-serif;">' +
    title +
    "</span>" +
    "</div>" +
...
function buildEmail(d) {
  ...
  return (
    ...
    section("Vehicle", [row("Vehicle Selected", cap(v(d, "vehicle")))]) +
    (hasFlightData
...
module.exports = async function (req, res) {
  ...
  try {
    await sendToResend(apiKey, {
      ...
      subject:
        "New Booking: " + (service || "Request") + (name ? " — " + name : ""),
      html: buildEmail(d),
    });
